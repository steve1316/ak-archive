...
const styles = {
	...
	// The art itself, zoomed past the slice and anchored high, since the illustrations are 2048px squares with the face in their upper third.
	art: {
		position: "absolute",
		inset: 0,
...
interface Layer {
	...
	id: number;
	/** The illustration, or null to fade the slice to empty. */
	url: string | null;
}
...
interface SkinSliceProps {
	/** The illustration this slice should show, or null for none. */
	url: string | null;
...
interface SkinBackdropProps {
	/** One illustration per operator on screen, left to right. Null leaves that slice empty. */
	urls: (string | null)[];
...
/**
 * One operator's band of the backdrop. A new skin waits until it has loaded, so a slow image never fades in half drawn, then fades in over
...
/**
 * The home carousel's backdrop: one band per operator on screen, each showing one of that operator's skins, feathered into its neighbours and
