/**
 * A small FlareSolverr client, copied from gfl-archive. The scheduled refresh runs FlareSolverr beside the job and sets `FLARESOLVERR_URL`, and
 * `wiki.mjs` then loads every wiki API page through one browser session. Local runs never set it and fetch directly.
 */
 ...
 *
 * The Arknights Wiki's Cloudflare can challenge datacenter addresses such as GitHub's runners, so the scheduled refresh loads its API pages in a
 * real browser instead. One session keeps the solved challenge for every request of the run.
 *
